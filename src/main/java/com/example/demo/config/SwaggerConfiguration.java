...
public class SwaggerConfiguration implements WebMvcConfigurer {
    ...
    public Docket airBottleDocket() {
        return (new Docket(DocumentationType.SWAGGER_2)).groupName("demo").apiInfo(this.getApiInfo()).useDefaultResponseMessages(false).globalResponseMessage(RequestMethod.GET, RESPONSE_MESSAGES).globalResponseMessage(RequestMethod.POST, RESPONSE_MESSAGES).globalResponseMessage(RequestMethod.PUT, RESPONSE_MESSAGES).globalResponseMessage(RequestMethod.DELETE, RESPONSE_MESSAGES).select().apis(RequestHandlerSelectors.basePackage("com.example.demo")).paths(PathSelectors.any()).build();
    }
